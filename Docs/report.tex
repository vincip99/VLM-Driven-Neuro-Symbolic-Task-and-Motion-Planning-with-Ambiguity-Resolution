...
\title{VLM-Driven Neuro-Symbolic Task and Motion Planning with Ambiguity Resolution\\
...
\begin{itemize}
    ...
    \item \textbf{BC Warm-Start Sample Efficiency:} With 100 demonstration trajectories, the BC-initialized policy enters the reward basin immediately, climbing to an optimal deterministic evaluation return of $199.41 \pm 53.39$ and a $\mathbf{95.0\%}$ task completion rate within only \textbf{25k environment steps} ($5\times$ faster convergence).
    \item \textbf{Stochastic Action Noise vs. Deterministic Evaluation:} During exploratory training rollouts, persistent Gaussian action noise ($\sigma \approx 1.0$) frequently disturbs micro-contacts at 20\,Hz, keeping the training rolling average around $\sim 50$ despite individual unperturbed rollouts achieving returns $>200$. In contrast, deterministic greedy evaluation ($\sigma = 0$) eliminates action jitter, revealing the true underlying $199.4$ peak return and $95.0\%$ task completion.
\end{itemize}
...
\begin{figure}[!t]
    ...
    \caption{Empirical comparison between the Deterministic Heuristic FSM controller and the learned PPO policy during cube manipulation rollouts across Yellow, Purple, and combined cube categories.}
...
\begin{itemize}
    \item \textbf{Execution Duration:} Both controllers exhibit nearly identical micro-trajectory durations: $1.72$\,s (34.3 steps) for the heuristic vs. $1.68$\,s (33.5 steps) for PPO. However, the heuristic requires an artificial $2.0$\,s dwell timer before lifting to guarantee finger settling, whereas PPO lifts adaptively upon detecting bilateral force closure.
    \item \textbf{Success Rate and Dynamic Compliance:} The PPO policy achieves a $\mathbf{92.5\%}$ overall grasp-and-lift success rate across multiple cube types ($\mathbf{95.0\%}$ on the trained Yellow Cube, and $\mathbf{90.0\%}$ zero-shot transfer on the Purple Cube, Fig.~\ref{fig:heuristic_vs_ppo}), compared to 100\% for the privileged analytical oracle.
...
Manipulation (Pick) & Hybrid PPO Policy & $1.68$\,s & 92.5\% (PPO alone) \\
Fallback Routine & Deterministic FSM & $1.72$\,s & 100\% (Combined) \\
\midrule
